...
func NewNodeManager(database *db.DB, nodeID string, opts ...Option) *NodeManager {
	...
	nm.dataSync = NewDataSyncManager(nm)
...
// OnReceiveData 处理收到的数据
func (nm *NodeManager) OnReceiveData(table string, key string, data any, timestamp int64) error {
	return nm.dataSync.OnReceiveData(table, key, data, timestamp)
}
...
// BroadcastData 广播数据
func (nm *NodeManager) BroadcastData(table string, key string, data any, timestamp int64) error {
	if nm.network == nil {
		return nil // 没有网络接口，跳过
	}
	
	return nm.network.BroadcastData(table, key, data, timestamp)
}

// FetchData 获取节点数据
func (nm *NodeManager) FetchData(sourceNodeID, tableName string) (map[string]map[string]any, error) {
	if nm.network == nil {
		return nil, ErrNoNetwork
	}
	
	return nm.network.FetchData(sourceNodeID, tableName)
}
