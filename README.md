...
GitHub Actions 工作流位于 `.github/workflows/ci.yml`，当前包含以下质量门禁：
...
- `lint`：`go vet ./...`
- `docs`：执行 `go run ./cmd/gen_sync_docs` 并校验 `docs/sync_protocol.md`、`docs/network_message.md` 已提交

同步最小端到端场景测试位于 `pkg/sync/sync_e2e_matrix_test.go`：

- 两节点 full sync
- 列级 delta 同步
- `gc_floor` 不一致时增量阻断
- `peer.gc_floor > local.gc_floor` 时自动 full sync 追平并恢复增量
